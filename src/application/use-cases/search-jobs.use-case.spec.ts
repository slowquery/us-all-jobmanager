...
describe('SearchJobsUseCase', () => {
  it('title 부분 일치(대소문자 무시)로 검색한다', async () => {
    ...
    repository.seed(makeJob({ id: 'b', title: 'Cleanup Logs' }));
  ...
  it('title과 status를 함께 지정하면 AND 조건으로 좁힌다', async () => {
    const repository = new InMemoryJobRepository();
    repository.seed(makeJob({ id: 'a', title: 'Deploy Service', status: 'pending' }));
    repository.seed(makeJob({ id: 'b', title: 'Deploy Service', status: 'completed' }));
    const useCase = new SearchJobsUseCase(repository);

    const jobs = await useCase.execute({ title: 'deploy', status: 'completed' });
