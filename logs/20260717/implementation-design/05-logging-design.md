...
공통 필드: `timestamp`(ISO 8601), `level`(`info`|`error`), `traceId`(형식: **32-hex** 소문자 —
06이 확정한 OTel **active span**의 트레이스 ID를 정본으로 하며, 요청/1 tick당 1개 발급), `spanId`
(선택, 스팬이 존재하는 경우에만 포함 — active span의 스팬 ID를 16-hex 소문자로 기록하고, 스팬이
없는 fallback 경로에서는 필드 자체를 생략한다), `source`(`http`|`scheduler`), `message`. 이후
이벤트별 전용 필드가 추가된다.
...
1) 요청 1건 (성공):
```json
{"timestamp":"2026-07-17T09:00:00.123Z","level":"info","traceId":"a1b2c3d4e5f60718293a4b5c6d7e8f90","spanId":"1a2b3c4d5e6f7081","source":"http","message":"request completed","method":"POST","path":"/jobs","statusCode":201,"latencyMs":12}
```

2) 스케줄러 배치 1건 (tick 종료 집계):
```json
{"timestamp":"2026-07-17T09:01:00.045Z","level":"info","traceId":"e5f6a7b8c9d0e1f2a3b4c5d6e7f80912","spanId":"2b3c4d5e6f708192","source":"scheduler","message":"tick completed","tickId":"e5f6a7b8c9d0e1f2a3b4c5d6e7f80912","batchSize":10,"succeeded":8,"failed":1,"skipped":0,"durationMs":340}
```

(스케줄러 tick의 `traceId`는 tick 루트 스팬의 트레이스 ID이며, `tickId`는 동일 tick을 가리키는
파생 필드로 항상 같은 값을 공유한다.)

3) 에러 1건 (PATCH 무효 전이):
```json
{"timestamp":"2026-07-17T09:02:11.900Z","level":"error","traceId":"c9d0e1f2a3b4c5d6e7f8091a2b3c4d5e","spanId":"3c4d5e6f70819213","source":"http","message":"request failed","method":"PATCH","path":"/jobs/b3f1...","statusCode":409,"errorCode":"INVALID_TRANSITION","latencyMs":5}
```

4) 상태 전이 이벤트 1건 (재시도 성공, `failed→pending`):
```json
{"timestamp":"2026-07-17T09:02:12.010Z","level":"info","traceId":"3f4a5b6c7d8e9f001122334455667788","spanId":"4d5e6f7081921354","source":"http","message":"transition committed","jobId":"b3f1...","from":"failed","to":"pending","actor":"api"}
```

5) 락 대기/보유 시간 1건 (임계구역 진입/이탈 측정):
```json
{"timestamp":"2026-07-17T09:02:12.008Z","level":"info","traceId":"3f4a5b6c7d8e9f001122334455667788","spanId":"4d5e6f7081921354","source":"http","message":"lock section measured","jobId":"b3f1...","waitMs":2,"holdMs":6}
```

(4·5는 같은 PATCH 요청의 처리 과정에서 발생한 두 이벤트이므로 `traceId`/`spanId`가 동일하며,
`jobId`도 교차 일관되게 `b3f1...`을 공유한다.)
...
- traceId 발급·전파 규약(형식·정본·전파 방식)은
  [06-observability-design.md](./06-observability-design.md)의 "traceId 발급·전파 규약 확정"
  절에서 확정되었다(본 문서는 그 형식을 로그 라인 필드로 반영).
