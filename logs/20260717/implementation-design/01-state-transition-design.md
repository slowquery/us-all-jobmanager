...
- guard 평가는 반드시 **[02-persistence-concurrency-design.md](./02-persistence-concurrency-design.md)가
  소유하는 임계구역(critical section) 내부**에서, 대상 job의 **최신 상태를 재조회한 뒤** 수행하고,
  guard가 참을 반환할 때만 쓰기를 수행해야 한다. 임계구역과 guard 평가 시점의 강제 주체는 02의
  `JobRepository` 구현체(`withTransition`)이며, 유스케이스는 이 포트를 호출하는 방식으로 규약을 경유한다.
...
- **application(+ports)**: `ProcessJobUseCase`(스케줄러 진입점), `PatchJobUseCase`(API 진입점)가
  02가 정의하는 `JobRepository` 포트(`withTransition`)를 호출해 atomic read→guard→write 규약 경유를
  orchestrate한다. `canTransition` 평가 자체는 유스케이스가 아니라 02 소유 임계구역 내부
  (infrastructure 구현체)에서 수행된다 — 유스케이스가 락 밖에서 guard를 선평가하는 것은 위 계약이
  금지하는 TOCTOU 경로다.
