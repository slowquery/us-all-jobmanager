...
### 동시성 문제 재현 테스트(baseline A/B)

**사용자 확정: C-1~C-5 전부 채택.** 위 "동시성 회귀 테스트" 절의 2건은 02·03이 이미 구현하기로
확정한 **보호 경로**(직렬화 큐 + guard-in-lock)가 실제로 동작하는지 검증하는 회귀(regression)다.
반면 본 절의 재현 테스트는 그 보호 경로를 적용하기 이전 상태를 흉내 낸 **"무보호"(unguarded)**
baseline 구현으로 동일 race를 재현해 문제가 실제로 발생함을 실증한 뒤, 보호 경로를 다시 씌워
해결됨을 대조하는 "채용 과제 서사"(재현→해결→기각안 실측) 목적이다. **역할 구분**: 회귀 테스트는
"항상 통과해야 하는 안전망"이고, 본 절의 baseline 재현 테스트는 "문제가 실재했음을 보여주는 데모"이며
서로 다른 assert 엄격도(아래 skip 규약 참조)를 가진다.

| # | 시나리오 | 결정론 확보 | assert 조건 |
| --- | --- | --- | --- |
| C-1 | **무보호 lost update A/B**: 02의 직렬화 큐를 우회한 "무보호" 리포지토리 구현(테스트 전용, `JsonDbJobRepository`의 큐를 거치지 않고 직접 read→guard→write)에 동일 PATCH 동시 2건을 발사해 lost update(둘 다 성공하거나 guard 우회)가 실제로 재현됨을 보인다. A(무보호, 재현 성공 기대)/B(보호 경로, 회귀 통과 기대) 두 실행을 대조. | 두 write 사이에 인위적 지연(`setTimeout`/Promise 지연 주입)을 read 이후 write 이전에 삽입해 race window를 결정론적으로 확대한다(지연 없이는 타이밍 의존 flaky). | A: 두 응답이 모두 2xx로 성공(guard 우회로 인한 lost update)해야 재현 성공. B(기존 회귀 테스트, 위 절)는 정확히 1건만 성공해야 한다 — 반대 결과가 나오면 재현 baseline이 실효성이 없거나 보호 경로가 깨진 것. |
| C-2 | **스냅숏 비일관(TOCTOU) 재현**: 무보호 리포지토리에서 read 스냅숏 이후 다른 트랜잭션이 끼어들어 stale 데이터로 write하는 경우를 재현한다. | 확률적 race이므로 동일 시나리오를 N회 반복 실행하고, 반복 중 최소 1회라도 비일관이 관측되면 재현 성공으로 간주한다(단일 실행으로는 결정론 확보 불가). | **skip 격리**: N회 반복 내내 비일관이 한 번도 재현되지 않으면 baseline 재현 테스트는 `test.skip`(사유 로그 기록)으로 처리해 CI를 막지 않는다 — 이 skip 규약은 baseline 재현 성공조건에만 적용되며, 보호 경로(위 회귀 테스트 절)의 assert는 재현 성공 여부와 무관하게 항상 하드 실패(never skip)다. |
| C-3 | **고부하 스트레스 불변식**: 무보호/보호 두 경로 각각에 N=50 동시 요청을 발사해, 최종 상태 불변식(잔여 job 수·상태 분포 합이 초기 시딩과 일치)이 유지되는지 확인한다. | N=50 고정, 시딩 데이터도 고정(테스트 내 상수)해 반복 실행 간 초기 조건을 동일하게 유지한다. | 보호 경로: 불변식(전이 총합·row 수) 100% 유지. 무보호 경로: 불변식 위반이 관측되어야 재현 성공(위반이 없으면 스트레스 강도 부족으로 재검토). |
| C-4 | **tick 중복(overrun) 재현**: `isTickRunning` 가드를 끈 구성(플래그 토글 구성 주입 — 03의 가드를 테스트 전용 설정으로 비활성화)으로 이전 tick 처리 중 다음 tick이 겹쳐 발화하는 경우를 재현해 중복 처리(동일 job 이중 선점 시도)를 관측한다. | 03의 `isTickRunning` 여부를 생성자/모듈 옵션으로 주입 가능하게 만들어(fake timer 없이) 동일 tick 함수를 겹쳐 `Promise.all`로 직접 호출하는 방식으로 결정론적으로 겹침을 발생시킨다. | 가드 끈 구성: 동일 job에 대해 두 tick 모두 선점을 시도한 흔적(guard 평가 2회 이상 또는 최종 상태 불일치)이 관측되어야 재현 성공. 가드 켠 구성(기본값, 03 정본): overrun 스킵 로그 1건 기록 + 중복 선점 없음. |
| C-5 | **크래시 유실 시뮬레이션(기각안 write-behind 실증)**: 02가 기각한 "write-behind"(비동기 지연 커밋) 대안을 테스트 더블로 구현해, 커밋 전 프로세스가 죽는 경우 데이터가 유실됨을 실증한다. | write-behind 더블(인메모리 버퍼 → 지연 flush)에서 flush 이전 시점에 강제로 예외를 던져 "크래시"를 시뮬레이션하고, 재기동 후 저장소를 재조회해 유실 여부를 확정적으로 판정한다(실제 프로세스 kill이 아닌 테스트 더블 내 결정론적 실패 주입). | write-behind 더블: flush 전 커밋 데이터가 재조회 시 사라져 있어야 재현 성공(기각 근거 실증). 02 채택안(즉시 write): 동일 시나리오에서 유실 없음이 대조군으로 확인되어야 한다. |

- **C-1 큐 우회 한정 규정**: C-1(및 C-2·C-3의 "무보호" 경로)이 사용하는 큐 우회 리포지토리 구현은
  **테스트 파일 내부에 한정**하며, `src/infrastructure` 실 코드에 무보호 경로를 추가하지 않는다.
  큐를 우회하는 모든 테스트 헬퍼 코드에는 "재현 테스트 전용 — 프로덕션 경로 아님" 사유 주석을
  필수로 남긴다.
- **C-4 토글 정당성**: `isTickRunning` 토글은 성능 최적화 파라미터의 정당한 DI 주입점이며(기본값은
  03 정본대로 가드 on), 무결성 계약(02 guard-in-lock)과 무관하다 — 실 코드에 무보호 데이터 경로를
  추가하는 것이 아니므로 위 "테스트 파일 한정" 규정과 상충하지 않는다.
- **C-5 Ponytail 사용자 확정 예외**: write-behind 더블은 Ponytail 사다리 1단(생략 가능) 판정
  대상이었으나, "재현→해결→기각안 실측" 서사 완결을 위해 사용자가 구현을 명시적으로 확정했다
  (post-interview 게이트 C, 통상적 생략 판정을 뒤집는 사용자 확정 예외로 본 절에 기록한다).
...
- **범위(In)**: 01의 전이 테이블 전 케이스(허용/불허), 02의 race 시나리오 2건(회귀) + C-1~C-5
  baseline 재현 5종(위 "동시성 문제 재현 테스트" 절), 03의 배치 처리 유스케이스 정상/실패 분기,
  04의 5개 엔드포인트 각각 성공·검증 실패·조회 실패(404) 경로.
- **비목표(Out)**: `isTickRunning`/overrun의 **실시간(real-time) 타이밍** 검증(예: 정확히 60초
  간격으로 발화하는지, `@nestjs/schedule` 프레임워크 배선 자체의 정밀도) — overrun **재현 자체**는
  C-4(플래그 토글 구성 주입, fake timer 미사용 결정론적 방식)로 비목표에서 해제되어 범위(In)에
  편입되었으므로, 여기서 제외되는 것은 어디까지나 실시간 타이밍 정밀도 측정이다. 06이 설계한
  트레이싱 스팬·로그 파이프라인의 자동화 검증(관측성은 설계 문서 산출물이며 코드 계측 자체가
  이번 세션 non-goal), 부하/성능 테스트(02의 처리량 상한은 설계 단계 추정치이며 실측 벤치마크는
  본 문서의 스코프 밖 — 단, C-3 N=50 스트레스 불변식 검증은 예외적으로 범위(In)), node-json-db
  라이브러리 자체의 동작(이미 02가 공식 문서 근거로 검증 완료, 재검증 불필요).
...
- `isTickRunning`/overrun의 **실시간** 타이밍 검증(스킵 재현 자체는 C-4로 커버됨, 위 비목표 절
  참조)이 필요해지면(예: 실제 운영 중 스킵 빈도·정밀 주기 이슈 발생 시) fake timer 기반 보조
  테스트를 별도로 추가하는 것을 재검토 후보로 남긴다.
...
- README에 "본인 코멘트"로 C-1~C-5 재현→해결→기각안(C-5) 실측 서사(baseline 실패 로그 →
  보호 경로 통과 대조)를 요약 링크하는 것은 구현 세션의 산출물로 남긴다.
