...
- (리뷰 반영) 레지스트리와 뉴스 라우팅 술어가 `readNewsDigestConfig().jobTitle`을 각각 독립적으로 읽으므로,
  구현 유형이 늘어나면 두 지점 동기화가 필요하다 — 향후 단일 상수/포트로 통합 검토.
- (리뷰 반영) "구현된 유형" 레지스트리는 처리기 구현 존재를 의미하며 런타임 게이팅(시크릿)과 별개다.
  뉴스 비활성 시 news-digest가 DefaultProcessor로 폴백(no-op)하는 경계를 ARCHITECTURE.md §1에 명시했다.
